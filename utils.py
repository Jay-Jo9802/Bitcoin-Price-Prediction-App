...
def make_window_custom(df_fe, scaler, harga_baru):
    ...
    new_row["EMA_10"] = pd.Series(
        pd.concat([df_temp["Close"], new_row["Close"]]).ewm(span=10).mean()
    ).iloc[-1]
    new_row["EMA_50"] = pd.Series(
        pd.concat([df_temp["Close"], new_row["Close"]]).ewm(span=50).mean()
    ).iloc[-1]
    ...
    new_row["return"] = (harga_baru - df_temp["Close"].iloc[-1]) / df_temp[
        "Close"
    ].iloc[-1]

    for w in [5, 10, 14, 20]:
        new_row[f"Close_mean_{w}"] = pd.Series(
            pd.concat([df_temp["Close"], new_row["Close"]]).rolling(window=w).mean()
        ).iloc[-1]
        new_row[f"Close_std_{w}"] = pd.Series(
            pd.concat([df_temp["Close"], new_row["Close"]]).rolling(window=w).std()
        ).iloc[-1]
        new_row[f"Volume_mean_{w}"] = pd.Series(
            pd.concat([df_temp["Volume"], new_row["Volume"]]).rolling(window=w).mean()
        ).iloc[-1]
